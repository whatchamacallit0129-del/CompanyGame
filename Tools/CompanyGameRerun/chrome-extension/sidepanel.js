const STORAGE_KEY = "companygame_rerun_state";
const state = {
  running: false,
  sequence: 0,
  taskId: "corridor-employee-movement",
  status: "stopped",
  lastError: "",
  lastResponse: "",
  lastResponseAt: 0,
  startedAt: 0
};
const $ = id => document.getElementById(id);

async function loadState() {
  const saved = await chrome.storage.local.get(STORAGE_KEY);
  if (saved[STORAGE_KEY]) Object.assign(state, saved[STORAGE_KEY]);
  render();
}

async function saveState() {
  await chrome.storage.local.set({ [STORAGE_KEY]: { ...state } });
}

function render() {
  $("status").textContent = state.status === "complete" ? "Rerun 완료" :
    state.status === "error" || state.status === "blocked" ? "Rerun 오류" :
    state.running ? "Rerun 실행 중" : "Rerun 정지";
  $("details").textContent = JSON.stringify(state, null, 2);
}

async function sendToChatGPT(prompt) {
  try {
    const result = await chrome.runtime.sendMessage({ type: "send_prompt", prompt });
    if (!result?.ok) throw new Error(result?.error || "메시지 전송 실패");
    state.lastError = "";
    await saveState();
    render();
    return result;
  } catch (error) {
    state.lastError = error?.message || String(error);
    state.status = "blocked";
    state.running = false;
    await saveState();
    render();
    throw error;
  }
}

async function startOrResume(isResume) {
  if (isResume) {
    state.running = true;
    state.status = "continue";
    state.lastError = "";
  } else {
    state.running = true;
    state.sequence += 1;
    state.taskId = "corridor-employee-movement";
    state.status = "continue";
    state.lastError = "";
    state.lastResponse = "";
    state.lastResponseAt = 0;
    state.startedAt = Date.now();
  }
  await saveState();
  render();

  const prefix = isResume ? "CONTINUE" : "START";
  await sendToChatGPT(`${prefix} — CompanyGame Rerun 작업 authorization.

현재 작업 범위는 게임적인 부분만이다.
Task ID: ${state.taskId}
Sequence: ${state.sequence}
목표: 통로(Node)와 직원 이동 시스템을 로보토미 코퍼레이션과 유사한 조작감으로 완성한다.

요구사항:
- 기존에 검증된 기능은 반복하지 않는다.
- 통로와 Node는 확장 가능한 소프트코딩 구조로 유지한다.
- 직원 선택 → 목적지 지정 → Node 경로 탐색 → Corridor를 따라 실제 이동 → 목적지 도착까지 구현한다.
- 이동속도, 가속/감속, 도착 거리, 그룹 간격 등은 설정 데이터로 분리한다.
- result.json과 error.json을 직접 확인하고 오류가 있으면 원인을 분석하여 수정한다.
- Unity에서 실제 직원이 연결된 Node를 순서대로 따라 목적지까지 이동하는 것을 검증하기 전에는 COMPLETE로 판단하지 않는다.
- 계속 작업해야 하면 응답에 CONTINUE를 포함한다.
- 모든 acceptance criteria를 실제로 통과하면 응답에 정확히 COMPLETE를 포함한다.

현재 GitHub 프로젝트 상태와 result/error JSON을 먼저 확인하고 현재 sequence의 미완료 지점부터 진행해.`);
}

async function processAssistantResponse() {
  if (!state.running || state.status !== "continue" || !state.lastResponse) return;
  if (state.lastResponseAt <= state.startedAt) return;

  const response = state.lastResponse;

  if (/\bCOMPLETE\b/i.test(response)) {
    state.running = false;
    state.status = "complete";
    await saveState();
    render();
    return;
  }

  if (/\bCONTINUE\b/i.test(response)) {
    state.sequence += 1;
    state.lastResponse = "";
    state.lastResponseAt = 0;
    state.status = "continue";
    await saveState();
    render();
    try {
      await sendToChatGPT(`CONTINUE — 같은 task(${state.taskId})의 sequence ${state.sequence}을 진행한다.
방금 응답 이후의 실제 미완료 지점부터 작업해.
GitHub 현재 상태와 result.json/error.json을 확인하고 검증된 작업은 반복하지 마.
실제 직원이 Node Graph를 따라 목적지까지 이동하는 것이 완료 조건이다.
오류가 있으면 직접 분석하고 수정한 뒤 다시 검증해.
완료되면 COMPLETE, 추가 작업이 필요하면 CONTINUE라고 명확하게 보고해.`);
    } catch (_) {}
  }
}

$("start").onclick = async () => { try { await startOrResume(false); } catch (_) {} };
$("resume").onclick = async () => { try { await startOrResume(true); } catch (_) {} };
$("stop").onclick = async () => {
  state.running = false;
  state.status = "stopped";
  await saveState();
  render();
};

chrome.runtime.onMessage.addListener(message => {
  if (message?.type !== "assistant_response") return;
  state.lastResponse = String(message.text || "");
  state.lastResponseAt = Number(message.timestamp || Date.now());
  saveState().then(() => processAssistantResponse()).catch(() => {});
});

loadState();
